...
<c:choose>
	<c:when test="<%= Validator.isNotNull(virtualHostname) %>">
		<aui:input label="set-the-robots-txt" helpMessage="robots-txt-help" name='<%= "TypeSettingsProperties--" + layoutSet.isPrivateLayout() + "-robots.txt--" %>' placeholder="robots" type="textarea" value="<%= layoutsAdminDisplayContext.getRobots() %>" />
	</c:when>
	<c:otherwise>
